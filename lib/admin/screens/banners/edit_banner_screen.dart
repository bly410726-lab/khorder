...
class _EditBannerScreenState extends State<EditBannerScreen> {
    ...
    return Column(
      ...
      children: [
        ...
        Text(
          _imageChanged
              ? 'New image selected'
              : 'Tap to replace image (JPEG, PNG, WebP, max 5MB)',
          style: TextStyle(
            fontSize: 12,
            color: _imageChanged ? AppColors.success : AppColors.textSecondary,
          ),
        ),
      ],
    );
  ...
  Widget _buildImagePreview() {
    ...
    final imageUrl = widget.banner.image;
  ...
  Future<void> _submit() async {
    ...
    final success = await provider.updateBanner(
      id: widget.banner.id ?? -1,
    ...
    if (!mounted) return;
    if (success) {
      Helpers.showSnackBar(context, 'Banner updated successfully');
      Navigator.pop(context);
    } else {
      Helpers.showSnackBar(
        context,
        provider.errorMessage ?? 'Could not update banner',
        isError: true,
      );
    }
  }
